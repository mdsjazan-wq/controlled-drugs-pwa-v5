/* app.js — v5
   تشغيل كامل: لوحة التحكم + التقارير + الإعدادات (مفعّلة)
...
function downloadText(filename, content, mime='text/csv'){
  ...
  a.href=URL.createObjectURL(new Blob([content],{type:mime})); a.download=filename;
  document.body.appendChild(a); a.click(); a.remove();
}

/* ---------- حالة التطبيق ---------- */
let CURRENT_USER=null, CURRENT_ROLE=null;
let MY_CENTERS=[], CURRENT_CENTER_ID=null;
...
$('btn-login')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    if(pass){ const {error}=await sb.auth.signInWithPassword({email,password:pass}); if(error) throw error; msg(m,'تم تسجيل الدخول ✅',true); }
    else{ const {error}=await sb.auth.signInWithOtp({email}); if(error) throw error; msg(m,'أرسلنا رابط الدخول إلى بريدك ✅',true); }
    await refreshAuthUI();
  }catch(e){ msg(m,e.message); }
});
$('btn-signup')?.addEventListener('click', async ()=>{
  const email=$('login-email')?.value?.trim(); const pass=$('login-password')?.value||''; const m=$('login-msg'); msg(m,'');
  try{
    ...
    msg(m,'تم إنشاء الحساب، تحقق من بريدك ✅',true);
  }catch(e){ msg(m,e.message); }
});
$('btn-logout')?.addEventListener('click', async ()=>{ await sb.auth.signOut(); CURRENT_USER=null; CURRENT_ROLE=null; MY_CENTERS=[]; CURRENT_CENTER_ID=null; await refreshAuthUI(); });

/* ---------- Tabs ---------- */
document.querySelectorAll('.chip')?.forEach(btn=>{
  btn.addEventListener('click',()=>{
    document.querySelectorAll('.chip').forEach(b=>b.classList.remove('active')); btn.classList.add('active');
    const tab=btn.dataset.tab; document.querySelectorAll('.tab').forEach(s=>s.classList.remove('active')); $('tab-'+tab)?.classList.add('active');
  });
});

/* ---------- Lookups: Items + My centers ---------- */
async function loadItems(){
  const ids=['issueItem','retItem','supplyItem','admin-wh-item-select','admin-ci-item'];
  try{
    const {data, error} = await sb.from('items').select('id,name').order('id');
    if(error) throw error;
    ids.forEach(id=>{ const el=$(id); if(!el) return; el.innerHTML=data.map(i=>`<option value="${i.id}">${i.name}</option>`).join(''); });
  }catch(e){ console.warn('items lookup:', e.message); }
}
async function loadMyCenters(){
  try{
    const {data, error}=await sb.rpc('rpc_list_my_centers'); if(error) throw error;
    MY_CENTERS = data||[];
    const def = MY_CENTERS.find(c=>c.is_default) || MY_CENTERS[0] || null;
    CURRENT_CENTER_ID = def ? def.id : null;
    const selects=['center-filter','issueCenter','retCenter','snapCenter','report-center','admin-ci-center'];
    selects.forEach(id=>{
      ...
      if(id==='center-filter' && MY_CENTERS.length<=1) el.parentElement?.style && (el.parentElement.style.display='none');
    });
...
async function loadWarehouseSnapshot(){
  ...
  try{
    const {data,error}=await sb.rpc('fn_warehouse_snapshot'); if(error) throw error;
    tb.innerHTML=''; (data||[]).forEach(r=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${r.item_name??''}</td><td>${r.on_hand??0}</td><td>${r.return_empty??0}</td><td>${r.return_expired??0}</td>`;
      tb.appendChild(tr);
    });
...
async function loadCenterSnapshot(){
  ...
  if(!CURRENT_CENTER_ID){ tb.innerHTML='<tr><td colspan="3" class="muted">لا يوجد مركز</td></tr>'; return; }
  try{
    const {data,error}=await sb.rpc('fn_center_snapshot',{p_center_id:CURRENT_CENTER_ID}); if(error) throw error;
  ...
  }catch(e){ tb.innerHTML=`<tr><td colspan="3" class="muted">${e.message}</td></tr>`; }
}
...
function ensureReportsUI(){
  ...
  // تعبئة المراكز
  const rsel=$('report-center'); if(rsel && MY_CENTERS.length){ rsel.innerHTML=MY_CENTERS.map(c=>`<option value="${c.id}">${c.name}</option>`).join(''); if(CURRENT_CENTER_ID) rsel.value=String(CURRENT_CENTER_ID); }
  ...
  $('btn-export-warehouse').onclick = async()=>{
    try{ const {data,error}=await sb.rpc('fn_warehouse_snapshot'); if(error) throw error; downloadText(`warehouse_${todayISO()}.csv`, toCSV(data||[])); }
    catch(e){ alert(e.message); }
  };
  $('btn-report-center').onclick = async()=>{
    const host=$('report-center-area'); const cid=+($('report-center')?.value||CURRENT_CENTER_ID||0);
  ...
  $('btn-export-center').onclick = async()=>{
    const cid=+($('report-center')?.value||CURRENT_CENTER_ID||0); if(!cid) return alert('اختر مركزًا');
    try{ const {data,error}=await sb.rpc('fn_center_snapshot',{p_center_id:cid}); if(error) throw error; downloadText(`center_${cid}_${todayISO()}.csv`, toCSV(data||[])); }
    catch(e){ alert(e.message); }
  };
}

/* ---------- Operations (Supply / Issue / Return) إذا وجدت عناصرها ---------- */
$('supplyBtn')?.addEventListener('click', async ()=>{
  const item=+$('supplyItem')?.value, qty=+$('supplyQty')?.value, date=$('supplyDate')?.value||todayISO(), el=$('supplyMsg'); msg(el,'');
  try{ const {error}=await sb.rpc('rpc_wh_receive_supply',{p_item_id:item,p_qty:qty,p_happened_at:date}); if(error) throw error;
       msg(el,'تم التوريد ✅',true); await loadWarehouseSnapshot(); }catch(e){ msg(el,e.message); }
});
$('issueBtn')?.addEventListener('click', async ()=>{
  const center=CURRENT_CENTER_ID||+$('issueCenter')?.value, item=+$('issueItem')?.value, qty=+$('issueQty')?.value, date=$('issueDate')?.value||todayISO(), el=$('issueMsg'); msg(el,'');
  try{ const {error}=await sb.rpc('rpc_wh_issue_to_center',{p_center_id:center,p_item_id:item,p_qty:qty,p_happened_at:date}); if(error) throw error;
       msg(el,'تم الصرف ✅',true); await Promise.all([loadWarehouseSnapshot(),loadCenterSnapshot()]); }catch(e){ msg(el,e.message); }
});
$('retBtn')?.addEventListener('click', async ()=>{
  const center=CURRENT_CENTER_ID||+$('retCenter')?.value, item=+$('retItem')?.value, qty=+$('retQty')?.value, status=$('retStatus')?.value||'empty', date=$('retDate')?.value||todayISO(), el=$('retMsg'); msg(el,'');
  try{ const {error}=await sb.rpc('rpc_wh_receive_return_from_center',{p_center_id:center,p_item_id:item,p_qty:qty,p_status:status,p_happened_at:date}); if(error) throw error;
       msg(el,'تم استلام المرتجع ✅',true); await Promise.all([loadWarehouseSnapshot(),loadCenterSnapshot()]); }catch(e){ msg(el,e.message); }
});

/* ---------- SETTINGS (Admin) ---------- */
function ensureSettingsUI(){
  const tab=$('tab-settings'); if(!tab) return;
  if(CURRENT_ROLE!=='admin'){ tab.innerHTML='<div class="card"><h3>الإعدادات</h3><div class="muted">هذه الصفحة مخصّصة لمدير النظام فقط.</div></div>'; return; }
  ...
  $('btn-admin-reset').onclick = async ()=>{
    ...
    try{ const {error}=await sb.rpc('rpc_admin_reset_all'); if(error) throw error; msg(m,'تم التصفير ✅',true); }
    catch(e){ msg(m,e.message); }
  };
  $('btn-admin-wh-save').onclick = async ()=>{
    const item=+$('admin-wh-item-select').value, qty=+$('admin-wh-qty').value, m=$('admin-wh-msg'); msg(m,'');
    try{
      // تفضيل admin API، وإلا fallback لو عندك الاسم القديم
      let res = await sb.rpc('rpc_admin_set_warehouse_initial',{p_item_id:item,p_qty:qty});
      if(res.error && /does not exist/i.test(res.error.message)) res = await sb.rpc('rpc_set_warehouse_initial',{p_item_id:item,p_qty:qty});
  ...
  $('btn-admin-ci-save').onclick = async ()=>{
    ...
    try{
      ...
      if(res.error && /does not exist/i.test(res.error.message)) res = await sb.rpc('rpc_set_center_initial',{p_center_id:center,p_item_id:item,p_qty:qty});
      if(res.error) throw res.error;
      msg(m,'تم الحفظ ✅',true);
    }catch(e){ msg(m,e.message); }
  };
...
async function refreshAdminCenters(){
  ...
  try{
    const {data,error}=await sb.rpc('rpc_admin_list_centers'); if(error) throw error;
    // عرض جدول بسيط
    ...
    (data||[]).forEach(r=>{
      h+=`<tr>
        ...
        <td>
          <button class="btn btn-light" data-del="${r.id}">حذف</button>
        </td>
      </tr>`;
    });
    ...
    host.querySelectorAll('[data-del]')?.forEach(btn=>{
      btn.onclick = async ()=>{
        ...
        const {error}=await sb.rpc('rpc_admin_delete_center',{p_center_id:id}); if(error) return alert(error.message);
    ...
    host.querySelectorAll('[data-activ]')?.forEach(ch=>{
      ch.onchange = async ()=>{
        ...
        const {error}=await sb.rpc('rpc_admin_save_center',{p_center_id:id,p_active:active,p_center_name:null}); if(error) return alert(error.message);
        await loadMyCenters(); // يحدث القوائم
      };
    });
  }catch(e){ host.innerHTML=`<div class="muted">${e.message}</div>`; }
}
...
async function refreshAdminUsers(){
  ...
  try{
    const {data,error}=await sb.rpc('rpc_admin_list_profiles'); if(error) throw error;
    // أدوار متاحة
    ...
    (data||[]).forEach(r=>{
      const centersOpts = MY_CENTERS.map(c=>`<option value="${c.id}" ${String(r.default_center_id||'')===String(c.id)?'selected':''}>${c.name}</option>`).join('');
    ...
    host.querySelectorAll('[data-save]')?.forEach(btn=>{
      btn.onclick = async ()=>{
        ...
        try{
          if(role){
            const {error}=await sb.rpc('rpc_admin_set_user_role',{p_user_id:uid,p_role:role}); if(error) throw error;
          }
          if(defc){
            const {error}=await sb.rpc('rpc_admin_set_user_default_center',{p_user_id:uid,p_center_id:+defc}); if(error) throw error;
          }
          alert('تم الحفظ ✅');
        }catch(e){ alert(e.message); }
      };
...
async function afterLoginInit(){
  ...
  await Promise.all([loadItems(), loadMyCenters()]);

  // اسم المستخدم
  try{
    const uid=(await sb.auth.getUser()).data.user?.id;
    if(uid){
      const {data:me}=await sb.from('profiles').select('full_name').eq('user_id',uid).maybeSingle();
